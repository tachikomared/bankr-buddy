...
function pushState(state, sessionId = "default") {
  ...
  if (mainWindow) {
    ...
    // Auto-enable mouse if happy or idle (for clicks)
    if (resolved === 'happy' || resolved === 'idle') {
...
ipcMain.on('disable-mouse', () => { 
  ...
  // We keep 'forward: true' so clicks can go through to other windows,
  // but ensure we don't accidentally ignore our own window if we're active.
  mainWindow?.setIgnoreMouseEvents(true, { forward: true }); 
});
...
ipcMain.on('show-sound-panel', () => { mainWindow?.webContents.send('show-sound-panel'); });
